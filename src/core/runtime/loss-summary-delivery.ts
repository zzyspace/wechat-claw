...
export interface LossDailySummaryDeliveryResult extends LossSummaryDeliveryResult {
  targetDate: string;
}

export interface LossWeeklySummaryDeliveryResult extends LossSummaryDeliveryResult {
  targetDate: string;
}
...
export async function sendLossDailySummary(input: {
  ...
  logger: Logger;
  targetDate?: string;
}): Promise<LossDailySummaryDeliveryResult> {
...
export async function sendLossWeeklySummary(input: {
  bot: WechatyInstance;
  channel: ChannelConfig;
  config: AppConfig;
  logger: Logger;
  targetDate?: string;
}): Promise<LossWeeklySummaryDeliveryResult> {
  const { bot, channel, config, logger } = input;

  if (!bot.isLoggedIn) {
    throw new Error(`Bot is not logged in. Weekly summary will not be sent for ${channel.code}.`);
  }
  ...
  const channelName = getChannelDisplayName(channel);
  const summaryText = renderLossWeeklySummaryForDate(targetDate, {
  ...
  if (deliveredTargets === 0) {
    throw new Error(`Weekly summary delivery failed for all targets on channel ${channel.code}`);
  }
