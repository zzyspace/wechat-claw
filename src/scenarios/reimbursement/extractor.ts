...
function buildPrompt(input: ReimbursementExtractionInput): string {
  return [
    ...
    "明确是食品原料、门店食材采购输出 food；明确是工资、薪资输出 salary；明确是房租、租金输出 rent；明确是水费、电费、水电费、水费账单、电费账单、电力缴费输出 utilities；明确包含“店长报账”字样输出 manager_reimbursement；明确包含“预报账”字样输出 planned_expense；非上述或不确定输出 other。",
