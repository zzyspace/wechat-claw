...
function buildPrompt(input: ReimbursementExtractionInput): string {
  return [
    ...
    "无论是否满足其他类别条件，只要明确包含“店长报账”字样一律输出 manager_reimbursement；否则，如果报账图片中的商品是鲜花、花卉、绿植、花材、花束、菊花、百合等花卉类型，或报账图片中出现“宏程下单号”字样，一律输出 flower；如果报账图片中出现“金辉”字样，或商户名称包含“泉州市丰泽区喜相逢百货商行”，一律输出 food；明确是食品原料、门店食材采购也输出 food；明确是工资、薪资输出 salary；明确是房租、租金输出 rent；明确是水费、电费、水电费、水费账单、电费账单、电力缴费输出 utilities；非上述或不确定输出 other。",
