...
test("extractReimbursementReport calls qwen3.5-flash and normalizes amount, date, and category", async () => {
  ...
  try {
    ...
    assert.match(promptText, /如果报账图片中出现“金辉”或“刘以琼”字样，或商户名称包含“泉州市丰泽区喜相逢百货商行”，一律输出 food/);
...
test("extractReimbursementReport forces planned_expense when OCR contains 李晨晨 three times", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "wechat-claw-reimbursement-extractor-"));
  const imagePath = path.join(tempDir, "receipt.jpg");
  ...
  try {
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  amount: "1580",
                  currency: "CNY",
                  expense_category: "manager_reimbursement",
                  voucher_date: "2026-07-15",
                  merchant: "微信转账",
                  document_no: null,
                  voucher_type: "转账截图",
                  ocr_text:
                    "Fuzzy 泉州-店长报账群(3) 李晨晨 转账 520 李晨晨 收款 530 李晨晨 汇款 530",
                  confidence: 0.89,
                }),
              },
            },
    ...
    assert.equal(result.resultJson.amount, 1580);
    assert.equal(result.resultJson.expenseCategory, "planned_expense");
    assert.equal(
      result.resultJson.ocrText,
      "Fuzzy 泉州-店长报账群(3) 李晨晨 转账 520 李晨晨 收款 530 李晨晨 汇款 530",
    );
    assert.equal(result.needsReview, false);
  } finally {
